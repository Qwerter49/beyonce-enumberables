...
def movies_after_2006 beyonceHash
    beyonceHash[:movies].select{|movie| movie[:year] >= 2006}
end


# 9. find a hit song where Beyonce's hair is blonde
def song_with_blonde_hair beyonceHash
    beyonceHash[:hits].find do
         |data| data[:hair].include?("blonde")
    end[:title]
end

# 15. get the average rating value for all of her movies
def average_rating_of_movies data 
    ratings = data[:movies].reduce(0) do |sum, movie|
        sum + movie[:rating]
    end
    ratings.to_f/data[:movies].length
end 

p average_rating_of_movies beyonceHash